...
class Node:
    ...
    def add_member(self, node_id, host, port, timestamp=None, sus=False, incarnation=0, callback=True):
        ...
        if callback and node_id != self.node_id: # callback to remap file
            for fun in self.add_member_callback:
                fun(node_id)
    ...
    def mark_node_as_failed(self, target_id):
        ...
        with self.lock:
            if target_id in self.membership_list:
                ...
                for fun in self.failure_callback:  # Notify hdfs of the failure
                        fun(target_id)
    ...
    def set_add_member_callback(self, callback):
        ...
        self.add_member_callback.append(callback)
    ...
    def set_failure_callback(self, callback):
        ...
        self.failure_callback.append(callback)
