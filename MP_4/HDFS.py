...
class Communicator:
    ...
    def reply_tcp_message(self, conn, response):
...
class HDFS:
    ...
    def handle_tcp_connection(self, conn, addr):
        ...
        try:
            ...
            # messages from RainStorm
            elif msg_type == 'CREATE':
                self.handle_rainstorm_create(conn, message)
            elif msg_type == 'APPEND':
                self.handle_rainstorm_append(conn, message)
            elif msg_type == 'GET':
                self.handle_rainstorm_get(conn, message)
            elif msg_type == 'MERGE':
                self.handle_rainstorm_merge(conn, message)
            elif msg_type == 'READ_FILE':
                self.handle_rainstorm_readfile(conn, message)
            else:
    ...
    def create(self, localfilename, hdfsfilename, localfilename_is_data=False):
        ...
        if os.path.exists(localfilename) or localfilename_is_data == True:
            if localfilename_is_data == True:
                file_data = localfilename.encode('utf-8')
            else: 
            ...
            def send_to_node(node_id):
                ...
                if node_id == self.node_id: # local create
                    ...
                    sent = self.communicator.send_and_receive_tcp_message(host, self.hdfs_port, message, timeout=100)
            ...
            print('HDFS_DEBUG: wait for Quorum')
            success_event.wait()
    ...
    def get(self, hdfsfilename, localfilename, return_data=False):
        ...
        def fetch_from_node(node_id):
            ...
            if node_id != self.node_id: # Need to get over the network
                ...
                response = self.communicator.send_and_receive_tcp_message(host, self.hdfs_port, message, timeout=100)
                if response and response.get('type') == 'FILE_DATA': # Fetch success
                    ...
                    with success_lock:
                        fail_count[0] += 1
                        if fail_count[0] == self.WRITE_CONSISTENCY_LEVEL():
                            success_event.set() # get failed but we need to unblock anyway
    ...
    def append(self, localfilename, hdfsfilename, localfilename_is_data=False):
        """Append data to a file in hdfs. Act as coordinator, send append to replicas"""
        if os.path.exists(localfilename) or localfilename_is_data == True:
            data_to_append = None
            if localfilename_is_data == True:
                data_to_append = localfilename.encode('utf-8')
            else: 
                with open(localfilename, 'rb') as f:
            ...
            def send_append(node_id):
                ...
                if node_id == self.node_id:  # local append
                    ...
                    ack = self.communicator.send_and_receive_tcp_message(host, self.hdfs_port, message, timeout=100)
    ...
    def user_input_loop(self):
        ...
        while self.alive:
            try:
                ...
                pass
    # modified for MP4 =========================================================================================================================
    def read_file(self, hdfsfilename):
        """Read the entire content of a file from HDFS and return it as a list of lines."""
        try:
            temp_local_file = f"temp_{hdfsfilename.replace('/', '_')}.txt"
            
            data = self.get(hdfsfilename, temp_local_file, return_data=True)
            if data:
                return data.splitlines() 
            else: 
                return None
        except Exception as e:
            print(f"[{self.node_id}] Error reading file '{hdfsfilename}': {e}")
            return None
    
    def handle_rainstorm_create(self, conn, message):
        self.create(message['localfile'], message['hdfsfile'], localfilename_is_data=message['use_data'])
        self.communicator.reply_tcp_message(conn, {'type': 'HDFS_ACK'})
        
    def handle_rainstorm_append(self, conn, message):
        self.append(message['localfile'], message['hdfsfile'], localfilename_is_data=message['use_data'])
        self.communicator.reply_tcp_message(conn, {'type': 'HDFS_ACK'})
        
    def handle_rainstorm_get(self, conn, message):
        self.get(message['hdfsfile'], message['localfile'])
        self.communicator.reply_tcp_message(conn, {'type': 'HDFS_ACK'})
        
    def handle_rainstorm_merge(self, conn, message):
        self.merge(message['hdfsfile'])
        self.communicator.reply_tcp_message(conn, {'type': 'HDFS_ACK'})
        
    def handle_rainstorm_readfile(self, conn, message):
        temp_local_file = f"temp_{message['hdfsfile'].replace('/', '_')}.txt"
        data = self.get(message['hdfsfile'], temp_local_file, return_data=True)
        if data:
            response = {
                'type': 'HDFS_ACK',
                'data': data.splitlines()
            }
            self.communicator.reply_tcp_message(conn, response)
        else: 
            response = {
                'type': 'HDFS_FAILED'
            }
